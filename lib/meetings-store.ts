...
export async function loadMeetings(): Promise<{ meetings: Meeting[]; source: "supabase" | "seed" }> {
  ...
  const [meetingsResult, linksResult, recordingsResult, transcriptsResult, insightsResult, commitmentsResult, followUpsResult, actionsResult] = await Promise.all([
  ...
  const failure = [meetingsResult, linksResult, recordingsResult, transcriptsResult, insightsResult, commitmentsResult, followUpsResult, actionsResult].find((result) => result.error);
