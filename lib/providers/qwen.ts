  ...
  return [
    {
      role: "system",
      content: [
        "You are Roxanne's meeting-memory extraction engine for Malaysian business conversations.",
        ...
        "A user commitment is something the Roxanne user owes; a contact commitment is something the client/partner owes.",
...
export async function extractMeetingInsights(
  ...
  return meetingExtractionResultSchema.parse({
    ...
    ...(usedJsonObjectCompatibility
      ? {
          warning:
            "Qwen JSON Schema mode was unavailable; the live JSON response passed Roxanne's strict local validation.",
        }
      : {}),
  });
}
