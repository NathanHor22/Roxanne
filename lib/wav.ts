...
export function parseLanternWav(data: Uint8Array): CanonicalWav {
  ...
  if (
    ...
    throw new Error("Lantern audio must be canonical 16 kHz mono 16-bit PCM WAV.");
  }
  const dataBytes = view.getUint32(40, true);
  if (dataBytes === 0 || dataBytes + 44 !== data.length || dataBytes % 2 !== 0) {
    throw new Error("Lantern WAV data length is invalid.");
  }
  return {
    sampleRate,
    channels,
