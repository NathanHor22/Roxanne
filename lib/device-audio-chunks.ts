...
/** Parses the byte range used by Lantern's idempotent SD-card uploads. */
export function parseDeviceContentRange(value: string | null): DeviceContentRange | null {
  ...
  if (!match) throw new Error("Lantern audio Content-Range is invalid.");
  const start = Number(match[1]);
  const end = Number(match[2]);
  const total = Number(match[3]);
  if (!Number.isSafeInteger(start) || !Number.isSafeInteger(end) ||
      !Number.isSafeInteger(total) || start > end || end >= total) {
    throw new Error("Lantern audio Content-Range is invalid.");
  }
...
export function deviceAudioPartName(offset: number) {
  if (!Number.isSafeInteger(offset) || offset < 0) {
    throw new Error("Lantern audio chunk offset is invalid.");
  }
