...
async function snapshotDay(client: SupabaseClient, userId: string, date: string, timezone: string): Promise<ReportMeeting[]> {
...
export async function prepareReportQuery(client: SupabaseClient, device: AuthenticatedLantern, input: ReportPlaybackInput) {
  ...
  if (request.action === "continue" || request.action === "repeat") {
    ...
    if (previous.error || !previous.data || page >= previous.data.pages.length) return storeSpeech(["That playback has expired. Ask for a fresh report and its date."]);
